...
uninit(void)
{
	...
	if(initflag == 1)
	{
		MY_LOGD("RT_refouces_step: deinit");
		dcs_rt_refocus_uninit();
        initflag = 0;
	}
    //[END]
    EXIT:
...
doImgProc(ImgInfo& img)
{
    ...
    switch(img.bufType)
    {
        case BufType_Display:
        {
            //[BEGIN]
            char mode_value[4];
			property_get("persist.camera.isp.mode",mode_value,"0");
			if (atoi(mode_value) == 1) 
			{
				MY_LOGD("BufType_Display ");
				mainDCS_RTRefocus(img);
			}
